...
public class SysIndexController extends BaseController {
    ...
    public String index(ModelMap mmap, HttpServletRequest request) {
        ...
        // 原有逻辑：如果 cookie 里有 menuStyle（兼容逻辑）
        Cookie[] cookies2 = ServletUtils.getRequest().getCookies();
        if (cookies2 != null) {
             for (Cookie cookie : cookies2) {
                if (StringUtils.isNotEmpty(cookie.getName()) && "menuStyle".equalsIgnoreCase(cookie.getName())) {
                    indexStyle = cookie.getValue();
                    break;
                }
            }
        }
        mmap.put("menuStyle", indexStyle);
        
        String webIndex = "topnav".equalsIgnoreCase(indexStyle) ? "index-topnav" : "index";

        // 设置默认首页为消防大屏
        mmap.put("main", "/fire/home");
